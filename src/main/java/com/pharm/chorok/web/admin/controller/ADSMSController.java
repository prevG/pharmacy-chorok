...
public class ADSMSController {
	...
	public void sendNaverCloudSms() {
		...
		String accessKey = "fPHUgJ5mJ4wNetD9qSpj";                     						// 네이버 클라우드 플랫폼 회원에게 발급되는 개인 인증키
		String secretKey = "8SluPZOG1YJUeqhD";  					// 2차 인증을 위해 서비스마다 할당되는 service secret
		String serviceId = "ncp-gov:sms:kr:706787115:edufine/messages";        				// 프로젝트에 할당된 SMS 서비스 ID
	    ...
	    toJson.put("subject","1");				// 메시지 제목 * LMS Type에서만 사용할 수 있습니다.
	    toJson.put("content","1");				// 메시지 내용 * Type별로 최대 byte 제한이 다릅니다.* SMS: 80byte / LMS: 2000byte
	    toJson.put("to","01030038397");					// 수신번호 목록  * 최대 50개까지 한번에 전송할 수 있습니다.
	    toArr.add(toJson);
	    ...
	    bodyJson.put("from","01030038397");				// 발신번호 * 사전에 인증/등록된 번호만 사용할 수 있습니다.		
	    bodyJson.put("subject","제목");				// 메시지 제목 * LMS Type에서만 사용할 수 있습니다.
	    bodyJson.put("content","내용");				// 메시지 내용 * Type별로 최대 byte 제한이 다릅니다.* SMS: 80byte / LMS: 2000byte
